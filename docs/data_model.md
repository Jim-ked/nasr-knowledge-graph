...
v1.0 的目标是建立可审计、可追溯的 NASR 静态空中运行路径源事实层。当前版本先把源表事实清洗成节点表、关系表和审计表，并可导入 Neo4j 源事实层；暂不生成路径搜索派生层。
...
Neo4j 入库脚本只导入上述源事实层节点和关系，不生成派生搜索边，也不提供机场间路径搜索主流程。后续若要做路径搜索，应在本源事实层通过审核后，再单独设计派生层。
