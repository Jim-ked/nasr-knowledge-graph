...
REQUIRED_TABLE_COLUMNS = {
    ...
    "DP_RTE.csv": {
        ...
        "BODY_SEQ", "TRANSITION_COMPUTER_CODE", "POINT_SEQ", "POINT",
        "POINT_TYPE", "NEXT_POINT",
    },
    "DP_APT.csv": {
        "DP_COMPUTER_CODE", "BODY_NAME", "BODY_SEQ", "ARPT_ID", "RWY_END_ID",
    },
    ...
    "STAR_APT.csv": {
        "STAR_COMPUTER_CODE", "BODY_NAME", "BODY_SEQ", "ARPT_ID", "RWY_END_ID",
    },
    "PFR_BASE.csv": {"ORIGIN_ID", "DSTN_ID", "PFR_TYPE_CODE", "ROUTE_NO"},
    "PFR_SEG.csv": {
...
def read_header(input_dir, filename):
    ...
    with path.open(encoding="cp1252", newline="") as handle:
        ...
        try:
            return {clean(column).upper() for column in next(reader)}
        except StopIteration:
            return set()


def validate_required_sources(input_dir):
    missing_tables = []
    missing_columns = {}
    for filename, required_columns in REQUIRED_TABLE_COLUMNS.items():
        header = read_header(input_dir, filename)
        if header is None:
    ...
    if missing_tables:
        messages.append(
            "Missing required NASR table(s): " + ", ".join(sorted(missing_tables))
        )
    for filename, columns in sorted(missing_columns.items()):
        messages.append(
            f"Missing required column(s) in {filename}: {', '.join(columns)}"
        )
    if messages:
...
def clear_output_files(*output_dirs):
...
def build_clean_graph_data(input_dir, clean_dir, audit_dir):
    input_dir = Path(input_dir)
    clean_dir = Path(clean_dir)
    audit_dir = Path(audit_dir)
    validate_required_sources(input_dir)
    clean_dir.mkdir(parents=True, exist_ok=True)
    audit_dir.mkdir(parents=True, exist_ok=True)
    clear_output_files(clean_dir, audit_dir)
...
def main():
    ...
    parser.add_argument("--clean-output", default="data/clean")
    parser.add_argument("--audit-output", default="data/audit")
    args = parser.parse_args()

    report = build_clean_graph_data(
